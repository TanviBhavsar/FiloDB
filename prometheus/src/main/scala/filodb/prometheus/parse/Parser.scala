...
object Parser extends StrictLogging {
  ...
  val file = new File("mismatchQueries.txt")
    ...
    modeParam match {
      ...
      case Shadow => {
        ...
        try {
          ...
          if (!expr.equals(antlrExpr)) {
            println(s"Antlr parsed expression does not match expression " +
              s"generated by Legacy Parser for query: $query")
            println(s"≈: $antlrExpr \n expr: $expr")

            logger.error(s"Antlr parsed expression does not match expression " +
              s"generated by Legacy Parser for query=$query, expr=$expr, antlrExpr=$antlrExpr")
          }
        } catch {
          case e: Throwable => {
            logger.error(s"Antlr parse error: $query", e)
  ...
  def queryRangeToLogicalPlan(query: String, timeParams: TimeRangeParams): LogicalPlan = {
    val lp1 = parseQueryWithPrecedence(query) match {
      case p: PeriodicSeries => p.toSeriesPlan(timeParams)
      case r: SimpleSeries   => r.toSeriesPlan(timeParams, isRoot = true)
      case _ => throw new UnsupportedOperationException()
    }
    val lp2 = parseQueryWithPrecedence(query, Antlr) match {
      case p: PeriodicSeries => p.toSeriesPlan(timeParams)
      case r: SimpleSeries   => r.toSeriesPlan(timeParams, isRoot = true)
      case _ => throw new UnsupportedOperationException()
    }

//    println("Legacy parser lp:" + lp1.toString)
//    println("ANtlr parser  lp:" + lp2.toString)

    if (!lp1.toString.replaceAll("\\s+","").
      equalsIgnoreCase(lp2.toString.replaceAll("\\s+",""))) {

      println("****Logical plan mismatch***")

     bw.write(s"query: $query")
      bw.write("\n")
      println("Legacy parser lp:" + lp1.toString)
      println("ANtlr parser lp:" + lp2.toString)
    }
