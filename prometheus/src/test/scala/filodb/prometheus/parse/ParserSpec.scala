...
class ParserSpec extends AnyFunSpec with Matchers {
  ...
  it("Should be able to make logical plans for Series Expressions") {
    val queryToLpString = Map(
      "sum by(namespace, cluster, label_spark_datastudio_apple_com_job_name, pod) (\n  label_replace(\n    label_replace(\n      container_memory_working_set_bytes{_ws_=\"aci-kubernetes\",_ns_=\"us-east-1a\",namespace=~\"spark-amp-ae-.+\",image=~\".+\",name!~\"POD\"},\n      \"exported_namespace\",\"$1\",\"namespace\", \"(.+)\"\n    ),\n    \"exported_pod\",\"$1\",\"pod\", \"(.+)\"\n  )\n    * on(exported_namespace, exported_pod) group_left(label_spark_datastudio_apple_com_job_name)\n  avg by(exported_namespace, exported_pod, label_spark_datastudio_apple_com_job_name)(\n    kube_pod_labels{_ws_=\"aci-kubernetes\",_ns_=\"us-east-1a\",exported_namespace=~\"spark-amp-ae-.+\",label_spark_datastudio_apple_com_job_name=~\".+\"}\n  )\n)  " -> ""
//" sum(\n  sum by(pod)(\n    rate(container_cpu_usage_seconds_total{_ws_=\"aci-kubernetes\",_ns_=\"us-west-2a\",namespace=\"amp-sre-genie-staging-west\",image!=\"\",container!=\"POD\"}[5m])\n  )\n    * on(pod) group_left\n  label_replace(\n    avg by(exported_pod)(\n      kube_pod_info{_ws_=\"aci-kubernetes\",_ns_=\"us-west-2a\",exported_namespace=\"amp-sre-genie-staging-west\",priority_class=~\"(p2|p4)\"}\n    ),\n    \"pod\", \"$1\", \"exported_pod\", \"(.+)\"\n  )\n)\n  /\nsum(kube_resourcequota{_ws_=\"aci-kubernetes\",_ns_=\"us-west-2a\",exported_namespace=\"amp-sre-genie-staging-west\",resource=\"requests.cpu\",type=\"used\",resourcequota=~\"(p2|p4)\"})" -> ""
   //   " sum (irate(container_cpu_usage_seconds_total{_ws_=\"aci-kubernetes\",_ns_=\"usmsc27n\",namespace=~\".*\",container!=\"POD\",container!=\"\"}[1h]) +\non(pod, namespace) group_left(priority_class)\nmax(\n  label_replace(\n    label_replace(\n      kube_pod_info{_ws_=\"aci-kubernetes\",_ns_=\"usmsc27n\",exported_namespace=~\".*\",priority_class=\"p3\"}, \"pod\", \"$1\", \"exported_pod\", \"(.*)\"\n    ), \"namespace\", \"$1\", \"exported_namespace\", \"(.*)\"\n  )\n) by (pod,priority_class,namespace) * 0)"-> ""
     // " 100 * (label_join(rate(container_cpu_usage_seconds_total{_ws_=\"aci-kubernetes\",_ns_=\"us-west-3a\",namespace=\"messaging-apns-perf\",container!=\"splunkforwarder\",pod=~\"bronxadmin.*\"}[5m]),\"podsvc\", \"-\", \"pod\") / on (podsvc) group_left label_join(max by (exported_pod) (kube_pod_container_resource_limits{_ws_=\"aci-kubernetes\",_ns_=\"us-west-3a\",exported_namespace=\"messaging-apns-perf\",exported_container!=\"splunkforwarder\",exported_pod=~\"bronxadmin.*\",resource=\"cpu\"}),\"podsvc\",\"-\",\"exported_pod\"))"-> ""
//"00 * (label_join(container_memory_max_usage_bytes{_ws_=\"aci-kubernetes\",_ns_=\"us-west-3a\",namespace=\"icloud-apns-malibu-prod\",container!=\"splunkforwarder\",pod=~\"malibu-shared.*\"},\"podsvc\", \"-\", \"pod\") / on (podsvc) group_left label_join(max by (exported_pod) (kube_pod_container_resource_limits{_ws_=\"aci-kubernetes\",_ns_=\"us-west-3a\",exported_namespace=\"icloud-apns-malibu-prod\",exported_container!=\"splunkforwarder\",exported_pod=~\"malibu-shared.*\",resource=\"memory\"}),\"podsvc\",\"-\",\"exported_pod\"))" -> ""
  //    "(sum by (service) (kube_persistentvolume_status_phase{_ws_=\"aci-kubernetes\",_ns_=\"us-east-1a\",phase!=\"Available\"} * on(persistentvolume) group_left(exported_node) (label_replace(kube_persistentvolume_labels{_ws_=\"aci-kubernetes\",_ns_=\"us-east-1a\"}, \"exported_node\", \"$1\", \"label_kubernetes_io_hostname\", \"(.+)\") and on (exported_node) kube_node_labels{_ws_=\"aci-kubernetes\",_ns_=\"us-east-1a\",label_machine_class=\"fast-storage\"} unless on (exported_node) kube_node_spec_taint{_ws_=\"aci-kubernetes\",_ns_=\"us-east-1a\",key!=\"dedicated\"}))\n) or vector(0)" -> ""
     // "sum by(namespace, cluster, label_spark_datastudio_apple_com_job_name, pod) (\n  label_replace(\n    label_replace(\n      kube_pod_container_resource_limits{_ws_=\"aci-kubernetes\",_ns_=\"us-west-3a\",exported_namespace=~\"spark-amp-ae-appstore-.+\",resource=\"cpu\",unit=\"core\"}\n        * on(exported_namespace, exported_pod) group_left(label_spark_datastudio_apple_com_job_name)\n      avg by(exported_namespace, exported_pod, label_spark_datastudio_apple_com_job_name)(\n        kube_pod_labels{_ws_=\"aci-kubernetes\",_ns_=\"us-west-3a\",exported_namespace=~\"spark-amp-ae-.+\",label_spark_datastudio_apple_com_job_name=~\".+\"}\n      ),\n      \"namespace\", \"$1\", \"exported_namespace\", \"(.+)\"\n    ),\n    \"pod\", \"$1\", \"exported_pod\", \"(.+)\"\n  )\n)" -> ""
//"sum by(namespace, cluster, label_spark_datastudio_apple_com_job_name, pod) (\n  label_replace(\n    label_replace(\n      kube_pod_container_resource_limits{_ws_=\"aci-kubernetes\",_ns_=\"us-west-1a\",exported_namespace=~\"spark-amp-ae-.+\",resource=\"cpu\",unit=\"core\"}" -> ""
//"""sum (bar1) + (sum (bar2) + sum (bar3) + sum (bar4))> 0""" -> ""
     // "label_replace(sum(rate(http_requests_total[10m])),instance,new-label,instance,\"(.*)-(.*)\")" -> ""
     //"label_replace(sum(rate(foo{_ns_=\"$.Values.cluster\"}[1m])) by (controller, method), \"controller\", \"$1\", \"controller\", \"(.*)Controller\")" -> ""
     //"label_replace(sum(rate(clouddriver_caching:controller:invocations__count_total{_ws_=\"playground-fuji\", _ns_=\"$.Values.cluster\"}[1m])) by (controller, method), \"controller\", \"$1\", \"controller\", \"(.*)Controller\")" -> ""
      // """sum( avg_over_time( play_play_time{_ws_="amp-qos",_ns_="vsq",pipeline_source="img",pipeline_id="vsq_prod_primary"} [5m] ) * 5 / 1000 )""" -> ""
//"""sum(foo)/sum(bar) """ -> ""
//      """sum( avg_over_time( play_bits{_ws_="amp-qos",_ns_="vsq",pipeline_source="img",pipeline_id="vsq_prod_primary"} [5m] ) * 5 / 1.024 ) /
//        |sum( avg_over_time( play_play_time{_ws_="amp-qos",_ns_="vsq",pipeline_source="img",pipeline_id="vsq_prod_primary"} [5m] ) * 5 / 1000 )""".stripMargin -> ""
   //   """sum(avg_over_time( play_bits{_ws_="amp-qos",_ns_="vsq",pipeline_source="img",pipeline_id="vsq_prod_primary"} [5m] ) * 5 / 1.024)"""-> ""
//      """100 - (
//        |  ( sum( play_volume_error{_ws_="amp-qos",_ns_="vsq",pipeline_source="img",pipeline_id="vsq_prod_primary"} ) /
//        |  sum( play_volume_start{_ws_="amp-qos",_ns_="vsq",pipeline_source="img",pipeline_id="vsq_prod_primary"} ) +
//        |  sum( play_volume_error{_ws_="amp-qos",_ns_="vsq",pipeline_source="img",pipeline_id="vsq_prod_primary"} )
//        |  ) * 100)""".stripMargin -> ""
...
//val file= new File("/Users/tanvibhavsar/Desktop/errors_in_logical.csv")
    val reader = CSVReader.open(new java.io.File("/Users/tanvibhavsar/Desktop/errors_in_logical.csv"))
  val res = reader.all()
   res.foreach { l => //println("query:" + l(0))
     val lp = Parser.queryToLogicalPlan(l(0), qts, step)
     //      if (lp.isInstanceOf[BinaryJoin]) printBinaryJoin(lp)

   }

//
//    val source = Source.fromFile("/Users/tanvibhavsar/test1.rtf")
//    for (line <- source.getLines()) {
//      val regex  = "^[0-9]+$";
//      //line.sp
//      val cols = line.split(regex).map(_.trim)
//
//      println("cols:" + cols(0))
